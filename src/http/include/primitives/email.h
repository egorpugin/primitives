...
struct Smtp
{
    ...
    String sendEmail(const Email &e) const
    {
        CURL *curl;
        ...
        curl_easy_setopt(curl, CURLOPT_READFUNCTION, detail::payload_source);

        detail::upload_status uctx;
        if (!e.custom_email)
        {
